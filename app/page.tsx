...
type NumberFactor = { kind: "number"; id: string; label: string; weight: number; unit?: string; domain: { min: number; max: number; better: "higher" | "lower" }; readOnly?: boolean; };
type ChecklistFactor = { kind: "checklist"; id: string; label: string; weight: number; cap?: number; items: { id: string; label: string; points: number }[] };
...
const CFG: Config = {
  factors: [
    { kind: "select", id: "facial_hair", label: "Facial Hair", weight: 0.01, options: [
      { label: "Clean shaven", value: 50 }, { label: "Stubble", value: 70 }, { label: "Trimmed beard", value: 85 }, { label: "Full beard", value: 95 },
    ]},
    { kind: "select", id: "chest_hair", label: "Chest Hair", weight: 0.01, options: [
      { label: "None", value: 70 }, { label: "Light", value: 80 }, { label: "Moderate", value: 90 }, { label: "Thick", value: 95 },
    ]},
    ...
    { kind: "checklist", id: "activities", label: "Activities completed", weight: 0.1, cap: 100, items: [
      { id: "hyrox", label: "HYROX", points: 20 }, { id: "spartan", label: "Spartan", points: 15 }, { id: "marathon", label: "Marathon", points: 20 },
      { id: "triathlon", label: "Triathlon", points: 20 }, { id: "murph", label: "Murph", points: 15 }, { id: "tough_mudder", label: "Tough Mudder", points: 15 },
      { id: "rock_climb", label: "Rock climbing", points: 10 }, { id: "snowmobiling", label: "Snowmobiling", points: 5 }, { id: "surfing", label: "Surfing", points: 10 },
      { id: "skiing", label: "Skiing", points: 10 }, { id: "snowboarding", label: "Snowboarding", points: 10 }, { id: "wakeboarding", label: "Wakeboarding", points: 10 },
      { id: "waterskiing", label: "Water skiing", points: 10 }, { id: "jetski", label: "Jet skiing", points: 5 }, { id: "shotgun", label: "Shotgun a beer", points: 5 },
      { id: "chopwood", label: "Chop wood", points: 5 }, { id: "bjj", label: "BJJ", points: 15 }, { id: "wrestling", label: "Wrestling", points: 15 },
      { id: "golfing", label: "Golfing", points: 5 }, { id: "boxing", label: "Boxing", points: 15 }, { id: "winfight", label: "Winning a fight", points: 20 },
      { id: "shootgun", label: "Shooting a gun", points: 10 }, { id: "shootbow", label: "Shooting a bow and arrow", points: 10 }, { id: "hiking", label: "Hiking", points: 5 },
      { id: "hockey", label: "Hockey", points: 10 }, { id: "lacrosse", label: "Lacrosse", points: 10 }, { id: "rugby", label: "Rugby", points: 15 },
      { id: "volleyball", label: "Volleyball", points: 5 }, { id: "powerlifting_meet", label: "Powerlifting meet", points: 20 }, { id: "football", label: "Football", points: 15 },
      { id: "motocross", label: "Motocross", points: 15 },
      { id: "fire_building", label: "Fire building", points: 10 }, { id: "fishing", label: "Fishing", points: 8 }, { id: "baby_making", label: "Baby Making", points: 5 },
    ]},
  ],
};
...
/** Compute from answers (for export if needed) */
function scoreFromAnswers(answers: Record<string, any>) {
  ...
  CFG.factors.forEach((f) => {
    if (f.kind === "number") {
      ...
      const total = (f as ChecklistFactor).items.reduce((sum, it) => sum + (set[it.id] ? it.points : 0), 0);
      const cap = (f as ChecklistFactor).cap ?? 100;
...
export default function Page() {
  ...
  function exportCSV() {
    ...
    Object.entries(us).forEach(([em, u]) => {
      ...
      rows.push([
        em, u.isAdmin ? "1" : "0", new Date(u.createdAt).toISOString(),
        u.profile?.name || "", u.profile?.profilePhoto ? "1" : "0", u.profile?.assessmentPhoto ? "1" : "0",
        alphaLook, "", ...factorVals,
      ]);
    });
    ...
    const a = document.createElement("a"); a.href = url; a.download = `alpha_status_export_${Date.now()}.csv`;
    document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
  }

  /** Number input (fix typing/focus) */
  function NumberInput({
    ...
    function commit(raw: string) {
      ...
      if (Number.isFinite(v)) {
        ...
        setText(String(clamped)); onChange(String(clamped));
  ...
  return (
    <main style={{
      ...
      {/* Background */}
      <div style={{ position: "fixed", inset: 0, backgroundImage: "url('/alpha-hero.png')", backgroundSize: "cover", backgroundPosition: "center top", filter: "grayscale(15%) contrast(1.05)", opacity: 0.25, zIndex: 0 }} />
      ...
      <header style={{ marginBottom: 16, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <Link href="/leaderboard"><button style={buttonGhost}>Leaderboard</button></Link>
          {currentEmail && (
            <>
              <button onClick={exportCSV} style={buttonGhost}>Export CSV</button>
              <button onClick={() => {
                setCurrentEmail(null); setAnswers({}); setName(""); setProfilePhoto(undefined); setAssessmentPhoto(undefined);
              }} style={buttonGhost}>Sign Out</button>
            </>
          )}
        </div>
      </header>
      ...
      {!currentEmail ? (
        <div style={{ ...box, display: "grid", gap: 12 }}>
          ...
          <label><div style={labelText}>Email</div><input style={inputStyle} value={email} onChange={(e) => setEmail(e.target.value)} /></label>
          <label><div style={labelText}>Password</div><input type="password" style={inputStyle} value={pwd} onChange={(e) => setPwd(e.target.value)} /></label>
          ...
          <div>
            <button
              onClick={async () => {
                ...
                if (isLoginMode) {
                  const u = users[email]; if (!u) return alert("No account found. Create one instead.");
                  if (u.passwordHash !== pwdHash) return alert("Wrong password.");
                  setCurrentEmail(email); setAnswers(u.answers || {});
                } else {
          ...
          <div style={{ display: "grid", gap: 12 }}>
            ...
            <details style={box}>
              ...
              <div style={{ marginTop: 12, display: "grid", gap: 12 }}>
                ...
                {[
                  { id: "facial_hair", label: "Facial Hair", options: [
                    { label: "Clean shaven", value: 50 }, { label: "Stubble", value: 70 }, { label: "Trimmed beard", value: 85 }, { label: "Full beard", value: 95 },
                  ]},
                  { id: "chest_hair", label: "Chest Hair", options: [
                    { label: "None", value: 70 }, { label: "Light", value: 80 }, { label: "Moderate", value: 90 }, { label: "Thick", value: 95 },
                  ]},
                  { id: "calloused_hands", label: "Calloused Hands", options: [
                    { label: "Soft", value: 50 }, { label: "Some", value: 80 }, { label: "Well-earned", value: 95 },
                  ]},
                  { id: "hand_size", label: "Hand Size", options: [
                    { label: "Small", value: 50 }, { label: "Medium", value: 75 }, { label: "Large", value: 90 }, { label: "Extra Large", value: 100 },
                  ]},
                ].map((f) => (
                  <label key={f.id}>
            ...
            <details style={box}>
              ...
              <div style={{ marginTop: 12, display: "grid", gap: 12 }}>
                {["street","academics","sports","financial","strength","politics","travel","survival","nutrition","first_aid","mechanics","navigation","cooking","home_repair","leadership","tech"]
                  .map((k) => {
          ...
          <div style={{ ...box, marginTop: 12 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                }} style={buttonPrimary}>Save</button>
